...
int DuplicateCheck (Node* pTest )
{
    Node* Duplicate = SearchBTree(pTree,pTest->Key);

            while (Duplicate != NULL)
            {
                cout<<"Key Entered is a duplicate, Please Enter another key... \n";
                cin>>pTest->Key;
                Duplicate = SearchBTree(pTree,pTest->Key);
            }
...
///////// Node insertation methods.

// 1- Insert using Refrence ---

// This will help of keeping track of Adresse and path to the node inside the tree.
...
int main()
{
    ...
    cout<< "\n=========== Inserting Nodes. ( We can use either Reference or 2 pointers) ==============\n" <<endl;

    for (int i = 0; i < 8; i++)
        //        InsertNode(pTree,NewNode());
        InsertNode2(pTree,pTree,NewNode());

        // testing Sequance is:  5,3,10,2,4,8,15,1
