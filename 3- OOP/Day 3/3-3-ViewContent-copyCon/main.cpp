...
public:
    ...
    MyStack (MyStack &S)
    {
        // passing by ref for the copy constructor input because:
        /*  Why (&) before (S) ??

            if we pass an object by value it calls a copy constructor...

            and because we created a copy constructor here it will call it and not
            the default.

            and when calling it will find it's passed by value and needs to call
            a copy constructor :D ... annnnnd the infinite loop goes on..
...
/*  no copy constructor
        ...
        a) this time we made a copy constructor which is summond anytime we input an
            object to a fn.
            and that copy ctor allocates new memory location.. so when it's job is done
            inside the fn. ... it free that location and not the location of S1

        b) so when S1 calls it's destructor at the end of the code. it finds the memory
            still allocated and it can free it without any error

        c) so basically the copy ctor helped us sepereate the memory locations.

            ctor -- 2 ( one for S1 and one for the copy ctor)
            dest. -- 2 for both. (one ends on fn. end and the other when code ends.)



        2- passing by reference.

        a) this is going to be the same as when we didn't had a copy ctor.
            the only object we will have is S1, but this is not ideal !!!

        b) as S1 data could be manipulated somehow inside the fn.
